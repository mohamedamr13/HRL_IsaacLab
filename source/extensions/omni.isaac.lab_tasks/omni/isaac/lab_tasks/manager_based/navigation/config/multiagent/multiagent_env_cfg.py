...
anymalc_cfg_scene_entity = SceneEntityCfg("Anymalc")
...
class EventCfg:
    ...
    reset_base_anymalc = RandTerm(
        func=mdp.reset_root_state_uniform,
        mode="reset",
        params={
            "pose_range": {"x": (-1, 1), "y": (1, 2), "yaw": (-3.14, 3.14)},
            "velocity_range": {
                "x": (-0.0, 0.0),
                "y": (-0.0, 0.0),
                "z": (-0.0, 0.0),
            ...
            "asset_cfg": anymalc_cfg_scene_entity 
        },
    )
...
class ObservationsCfg:
    ...
    @configclass
    class AnymalCPolicyCfg(ObsGroup):
        """Observations for policy group."""

        # observation terms (order preserved)
        base_lin_vel = ObsTerm(func=mdp.base_lin_vel,params={ "asset_cfg": anymalc_cfg_scene_entity }  )
        base_ang_vel = ObsTerm(func=mdp.base_ang_vel, params={ "asset_cfg": anymalc_cfg_scene_entity } )
        projected_gravity = ObsTerm(func=mdp.projected_gravity,params={ "asset_cfg": anymalc_cfg_scene_entity } )
        # pose_command = ObsTerm(func=mdp.generated_commands, params={"command_name": "anymalc_base_velocity"})
        # height_scan = ObsTerm(
        #     func=mdp.height_scan,
        #     params={"sensor_cfg": SceneEntityCfg("height_scanner_anymalc")},
        #     noise=Unoise(n_min=-0.1, n_max=0.1),
        #     clip=(-1.0, 1.0),
        # )
        # camera_capture = ObsTerm(
        ...
        velocity_commands = ObsTerm(func=mdp.generated_commands, params={"command_name": "anymalc_base_velocity"})
        joint_pos = ObsTerm(func=mdp.joint_pos, params={ "asset_cfg": anymalc_cfg_scene_entity }, noise=Unoise(n_min=-0.01, n_max=0.01), clip=(-1.0, 1.0) )
        joint_vel = ObsTerm(func=mdp.joint_vel, params={ "asset_cfg": anymalc_cfg_scene_entity }, noise=Unoise(n_min=-1.5, n_max=1.5), clip=(-1.0, 1.0))
        actions = ObsTerm(func =mdp.last_action, params={ "action_name": 'anymalc_pre_trained_policy_action' } )

    @configclass
    class MultiAgentPolicyCfg(ObsGroup):
        "Observations for policy group."
        base_lin_vel_anymalc = ObsTerm(func=mdp.base_lin_vel, params={ "asset_cfg": anymalc_cfg_scene_entity }  )
        
        projected_gravity_anymalc = ObsTerm(func=mdp.projected_gravity, params={ "asset_cfg": anymalc_cfg_scene_entity } )
        
        pose_command_anymalc = ObsTerm(func=mdp.generated_commands, params={"command_name": "anymalc_pose_command"})
        # height_scan_anymalc = ObsTerm(
        #     func=mdp.height_scan,
        #     params={"sensor_cfg": SceneEntityCfg("height_scanner_anymalc")},
        #     noise=Unoise(n_min=-0.1, n_max=0.1),
        #     clip=(-1.0, 1.0),
    ...
    anymalc_policy: AnymalCPolicyCfg = AnymalCPolicyCfg()
...
class ActionsCfg:
    ...
    anymalc_pre_trained_policy_action: mdp.PreTrainedPolicyActionCfg = mdp.PreTrainedPolicyActionCfg(
        asset_name="Anymalc",
        policy_path=f"{ISAACLAB_NUCLEUS_DIR}/Policies/ANYmal-C/Blind/policy.pt",
        low_level_decimation=4,
        low_level_actions=mdp.JointPositionActionCfg(asset_name="Anymalc", joint_names=[".*"], scale=0.5, use_default_offset=True),
        low_level_observations=multiagent_cfg.anymalc_policy,
    )
...
class RewardsCfg:
    ...
    _, position_tracking_go1, position_tracking_fine_grained_go1, orientation_tracking_go1 = generate_reward_terms("go1_pose_command")
    termination_penalty, position_tracking_anymalc, position_tracking_fine_grained_anymalc, orientation_tracking_anymalc = generate_reward_terms("anymalc_pose_command")
...
class CommandsCfg:
    ...
    anymalc_pose_command = mdp.UniformPose2dCommandCfg(
        asset_name="Anymalc",
        simple_heading=False,
        resampling_time_range=(8.0, 8.0),
        debug_vis=True,
    ...
    anymalc_base_velocity = mdp.UniformVelocityCommandCfg(
        asset_name="Anymalc",
...
class TerminationsCfg:
    ...
    anymalc_base_contact = DoneTerm(
        func=mdp.illegal_contact,
        params={"sensor_cfg": SceneEntityCfg("anymalc_contact_forces", body_names="base"), "threshold": 1.0},
    )
...
class MySceneCfg(InteractiveSceneCfg):
    ...
    Anymalc: ArticulationCfg = ANYMAL_C_CFG.replace(prim_path="{ENV_REGEX_NS}/AnymalC")
    # sensors
    ...
    height_scanner_anymalc = RayCasterCfg(
            prim_path="{ENV_REGEX_NS}/AnymalC/base",
            offset=RayCasterCfg.OffsetCfg(pos=(0.0, 0.0, 20.0)),
            attach_yaw_only=True,
            pattern_cfg=patterns.GridPatternCfg(resolution=0.1, size=[1.6, 1.0]),
            debug_vis=True,
...
@configclass
class MutliAgentNavigationEnvCfg(ManagerBasedRLEnvCfg):
    ...
    def __post_init__(self):
        ...
        if self.scene.go1_contact_forces is not None:
            ...
            self.scene.anymalc_contact_forces.update_period = self.sim.dt

        # self.scene.camera = camera


class MultiAgentNavigationEnvCfg_PLAY(MutliAgentNavigationEnvCfg):
